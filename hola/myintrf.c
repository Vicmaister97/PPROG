...
void extra_write_message_object_intrf(intrf *ic, char * mg){
	...
	if(strlen(mg) > ic->cols - ic->menu_cols -10){
		...
		int w = aux - mg, i = 0;
		char *smth = (char *) malloc(sizeof(char)*w + 1);
		for( ; i < w; i++)
			smth[i] = mg[i];
		smth[i] = '\0';
		win_write_line_at(ic->extra, 2, 2, smth);
		win_write_line_at(ic->extra, 3, 2, aux+1);
...
void display_inventory(intrf *ic, char *pict, char **names, int num){
	...
	for( ; i < num; i++){
		...
		if(i+1 < ic->extra_rows-3)
			win_write_line_at(ic->extra, i+1, 2, buf);
		else if(j+1 < ic->extra_rows-3){
